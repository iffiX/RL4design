...
public:
    ...
    template <typename U> explicit Vec3D<T>(const Vec3D<U>& s)		        {x = (T)s.x; y = (T)s.y; z = (T)s.z;} //!< Copy constructor from another template type
    ...
    template <typename U> Vec3D<T> operator+(const Vec3D<U>& s)		        {return Vec3D<T>(x+s.x, y+s.y, z+s.z);} //!< addition operator for different template types
    template <typename U> Vec3D<T> operator-(const Vec3D<U>& s)		        {return Vec3D<T>(x-s.x, y-s.y, z-s.z);} //!< subtraction operator for different template types
    template <typename U> Vec3D<T> operator*(const U& f) const		        {return Vec3D<T>(f*x, f*y, f*z);} //!< multiplication operator for different template types
    template <typename U> friend Vec3D<T> operator*(const U f, const Vec3D<T>& v) {return v*f;} //!< multiplication operator for different template types with number first. Therefore must be a friend because scalar value comes first.
    ...
    Vec3D operator+(const Vec3D &v) const		    {return Vec3D(x+v.x, y+v.y, z+v.z);} //!< overload addition.
    Vec3D operator-(const Vec3D &v) const		    {return Vec3D(x-v.x, y-v.y, z-v.z);} //!< overload subtraction.
    Vec3D operator-() const					        {return Vec3D(-x, -y, -z);} //!< overload negation (unary).
    Vec3D operator*(const T &f) const			    {return Vec3D(f*x, f*y, f*z);} //!< overload multiplication.
    friend Vec3D operator*(const T f, const Vec3D& v) {return v*f;} //!< overload multiplication with number first.
    Vec3D operator/(const T &f) const			    {T Inv = (T)1.0/f; return Vec3D(Inv*x, Inv*y, Inv*z);} //!< overload division.
