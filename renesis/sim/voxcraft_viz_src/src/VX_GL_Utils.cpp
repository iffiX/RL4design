...
static constexpr vfloat pi = 3.14159265358979323846;
...
// --------------------------------------------------------------------------
{
    GLuint boxFaceList = glGenLists(1);
    glNewList(boxFaceList, GL_COMPILE);
    glBegin(GL_QUADS);
    if (!Topless) {
    ...
    glEndList();
    glCallList(boxFaceList);
    glDeleteLists(boxFaceList, 1);
}
...
// --------------------------------------------------------------------------
{
    GLuint boxEdgeList = glGenLists(1);

    glNewList(boxEdgeList, GL_COMPILE);
    ...
    glEndList();
    glCallList(boxEdgeList);
    glDeleteLists(boxEdgeList, 1);
}
...
// --------------------------------------------------------------------------
{
    ...
    static constexpr vfloat da = pi / 12;
                               ...
                               const CColor &Color) // draws 3D arrow Directly from vO to vP
{
    ...
    for (auto & i : Perp) {
        glVertex3d(vP.x, vP.y, vP.z);
        glVertex3d(vP.x + HeadBackPoint.x + i.x, vP.y + HeadBackPoint.y + i.y, vP.z + HeadBackPoint.z + i.z);
